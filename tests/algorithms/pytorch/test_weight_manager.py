...
def test_get_parameters(torch_linear_model):
    ...
    parameters = weight_manager.get_parameters(
        model=model, with_batch_norm_parameters=False
    )
...
def test_get_batch_norm_layer(batch_norm_cnn):
    ...
    model_parameters = list(
        weight_manager.get_parameters(model=model, with_batch_norm_parameters=True)
    )
    ...
    # By default the running mean is set to 0 and the running variance to 1 : https://pytorch.org/docs/stable/generated/torch.nn.BatchNorm2d.html
...
def test_subtract_parameters(model, with_batch_norm_parameters, request):
    ...
    subtracted_parameters = weight_manager.subtract_parameters(
        weight_manager.get_parameters(
            my_model1, with_batch_norm_parameters=with_batch_norm_parameters
        ),
        weight_manager.get_parameters(
            my_model2, with_batch_norm_parameters=with_batch_norm_parameters
        ),
    )
...
def test_increment_parameters(model, with_batch_norm_parameters, request):
    ...
    parameters1 = weight_manager.get_parameters(
        my_model1, with_batch_norm_parameters=with_batch_norm_parameters
    )
    parameters2 = weight_manager.get_parameters(
        my_model2, with_batch_norm_parameters=with_batch_norm_parameters
    )
