...
def test_default_logging_level(caplog):
    ...
    logger = logging.getLogger(name="substrafl.junior")
    with caplog.at_level(logging.DEBUG):
        logger.info("This should be logged")
        logger.debug("This should not be logged")

        assert logging.getLogger("substrafl.junior").getEffectiveLevel() == logging.INFO
...
def test_set_logging_level(caplog):
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        assert logging.getLogger("substrafl.junior").getEffectiveLevel() == logging.WARNING
        assert len(caplog.records) == 1
